...
. $PSScriptRoot\scripts\luis_functions.ps1
. $PSScriptRoot\scripts\qna_functions.ps1

foreach ($filePath in $config_files -split ",")
{
    $file = Get-Item -Path $filePath
    $fileName = $file | % { $_.BaseName } 
    ...
    {
        # Update LUIS apps
        foreach ($app in $config.cognitiveModels | Where { $_.type -eq "luis" })
        {
            luis export version `
                --appId $app.appid `
        ...
        foreach ($kb in $config.cognitiveModels | Where { $_.type -eq "qna" })
        {          
            qnamaker export kb `
                --environment Prod `
                --kbId $kb.kbId `
    ...
    foreach ($dispatch in $config.cognitiveModels | Where {$_.type -eq "dispatch"})
    {
